-- Script para configurar sistema de administradores en Supabase (versión corregida)
...
-- 2. Crear función para actualizar updated_at automáticamente (si no existe)
...
-- 3. Eliminar trigger existente si existe y crear uno nuevo
...
-- 4. Crear función para verificar credenciales de admin (si no existe)
...
-- 5. Crear función para crear admin (si no existe)
...
-- 6. Crear función para cambiar contraseña (si no existe)
...
-- 7. Crear tabla de logs de administradores (si no existe)
...
-- 8. Crear función para registrar logs (si no existe)
...
-- 9. Crear función para verificar si un admin existe (sin validar contraseña)
...
-- 10. Crear función para obtener logs con información del admin (si no existe)
...
-- 10. Configurar políticas de seguridad (RLS)
...
-- 11. Crear índices para mejor rendimiento (si no existen)
...
-- 12. Insertar admin inicial solo si no existe
INSERT INTO administradores (username, password_hash, nombre, email)
SELECT 'admin', crypt('excusas2025', gen_salt('bf')), 'Administrador Principal', 'admin@excusas.com'
WHERE NOT EXISTS (SELECT 1 FROM administradores WHERE username = 'admin');
